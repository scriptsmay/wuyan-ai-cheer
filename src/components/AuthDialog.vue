...
import { LogIn, LogOut, ShieldCheck, X, AlertCircle } from '@lucide/vue';
...
<template>
  ...
  >
    <section class="auth-panel">
      <button class="icon-button auth-close" type="button" aria-label="关闭登录面板" @click="emit('close')">
        <X :size="20" />
      </button>
      ...
      <form v-if="!isAuthenticated" class="auth-form" @submit.prevent="submit">
        <label
          >用户名<input v-model="loginUsername" autocomplete="username" maxlength="64" placeholder="请输入用户名"
        /></label>
        <label
          >密码<input
            v-model="password"
      ...
      <button v-else class="ghost-button wide" type="button" :disabled="loading" @click="logout">
        <LogOut :size="17" /> 退出登录
      </button>
