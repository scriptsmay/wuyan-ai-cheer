...
<template>
  <section class="result-workbench">
    <div class="result-copy-column">
      ...
      <textarea v-model="editingLine" class="line-editor" rows="3" maxlength="80" placeholder="编辑应援文案…" />
      <span class="edit-char-count">{{ editingLine.length }}/80</span>

      <label class="edit-field-label">心情标语（可编辑）</label>
      <input v-model="editingCaption" class="caption-editor" maxlength="30" placeholder="编辑心情标语…" />
    ...
    <div class="card-preview-column">
      ...
      <div v-if="card" class="card-toggles">
        <label class="qr-toggle" :class="{ disabled: cardLoading }">
          <span
            ...
            :aria-checked="showQr"
            tabindex="0"
            @click="cardLoading || (showQr = !showQr) || handleRender()"
          >
            <input
              v-model="showQr"
              type="checkbox"
        ...
        <label v-if="result.refs.length" class="qr-toggle" :class="{ disabled: cardLoading }">
          <span
            ...
            tabindex="0"
            @click="cardLoading || (showRefs = !showRefs) || handleRender()"
          >
            <input
              v-model="showRefs"
              type="checkbox"
      ...
      <button
        v-if="card && !requiresLongPressSave()"
