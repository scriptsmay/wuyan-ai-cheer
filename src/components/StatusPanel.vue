...
import { AlertTriangle, LoaderCircle, RadioTower, RefreshCw } from '@lucide/vue';
...
<template>
  <section class="status-panel" :class="`status-${status}`" aria-live="polite">
    ...
    <AlertTriangle
      v-else-if="status === 'rate-limited' || status === 'network-error' || status === 'service-error'"
      :size="30"
    />
    ...
    <div>
      ...
      <p>
        {{
          message ||
          (status === 'loading' ? '正在读取最新数据并组织文案……' : '选择一种心情，生成可复制的文案和 3:4 应援卡。')
        }}
      </p>
    </div>
