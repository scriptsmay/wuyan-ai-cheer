...
function drawSignalHeader(context: CanvasRenderingContext2D, checkin?: Checkin) {
  ...
  if (checkin) {
    ...
    context.fillText(`${checkin.date}  ·  连续 ${checkin.streak} 天  ·  累计 ${checkin.total_days} 天`, 146, 300)
  }
}

function drawMainCopy(context: CanvasRenderingContext2D, line: string, emojiCaption: string) {
  context.fillStyle = '#00D4FF'
  context.font = '500 24px "Noto Sans SC"'
  context.fillText('SIGNAL / 01', 92, 515)
  context.fillStyle = '#F5FBFF'
  context.font = '700 76px "Noto Sans SC"'
  const rows = wrapText(context, line, 840)
  rows.slice(0, 4).forEach((row, index) => context.fillText(row, 92, 620 + index * 106))
...
function formatDateTime(value: string): string {
  if (!value || Number.isNaN(Date.parse(value))) return '暂无快照时间'
  return new Intl.DateTimeFormat('zh-CN', {
    timeZone: 'Asia/Shanghai',
    year: 'numeric',
    month: '2-digit',
