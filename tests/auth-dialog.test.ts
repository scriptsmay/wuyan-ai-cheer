...
function mountDialog(props: { open: boolean; mode: AuthMode; username: string }) {
  return mount(AuthDialog, { props, attachTo: document.body });
}

async function fillAndSubmit(wrapper: ReturnType<typeof mountDialog>, username: string, password: string) {
...
describe('AuthDialog submit (real component)', () => {
  ...
  it('blocks submission when clearSession throws and surfaces error message', async () => {
    clearSessionMock.mockRejectedValueOnce(new Error('退出登录失败：旧会话仍驻留，请重试'));
